...
export class CarModelResolverBase {
  ...
  async carModels(
    ...
    return this.service.carModels(args);
  }
  ...
  async carModel(
    ...
    const result = await this.service.carModel(args);
    if (result === null) {
      return null;
    }
    return result;
  ...
  async deleteCarModel(
    ...
    try {
      return await this.service.deleteCarModel(args);
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new GraphQLError(
          `No resource was found for ${JSON.stringify(args.where)}`
  ...
  async findParts(
    @graphql.Parent() parent: CarModel,
  ...
  @graphql.Mutation(() => CarModelCreateInput)
  async CreateCarModel(
    @graphql.Args()
    args: CarModelCreateInput
  ): Promise<CarModelCreateInput> {
    return this.service.CreateCarModel(args);
  }
}
